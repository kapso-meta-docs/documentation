...
![Image](https://scontent-lga3-2.xx.fbcdn.net/v/t39.2365-6/569169662_1344528437405807_2132705829768077708_n.jpg?_nc_cat=107&ccb=1-7&_nc_sid=e280be&_nc_ohc=_kR4ThUfEUIQ7kNvwELWuEi&_nc_oc=AdmOqkgrGY6z2fp5vzIsvfsXPk9zdA-CAz87IBQvuJfvBzsrWWC4ER560rlIBoEsNdM&_nc_zt=14&_nc_ht=scontent-lga3-2.xx&_nc_gid=xGSyuZjINTR8UQ-O65MLkw&oh=00_AfkUFqTR4P8rRRkklNog7jhwSo5oT_FvXNg1wGFrAY6x5g&oe=69551822)
...
Obtain the merchant vpa, mcc and purpose code from Cashfree. This will be used to set up the [payment configuration](https://developers.facebook.com/docs/whatsapp/on-premises/payments-api/upi#before-you-start) on WhatsApp. Multiple vpas are supported and hence multiple payment configurations can be set up. Each payment configuration should have one VPA.
...
[Reference doc](https://l.facebook.com/l.php?u=https%3A%2F%2Fdocs.cashfree.com%2Freference%2Fcreateorder&h=AT2PbFbrfwnt3znOaYe8wjq2g-CvXIog9xs1fqLByx0UojWZOVJVngUsZQAGWCrR6yzCyNPtb4Jt7rkmKrbhAPmbJuiM08-KFvA-8emqu2TYCT6zDVbKbJGUSepDkS76uz77Qd__JjYujQ)
...
[Reference doc](https://l.facebook.com/l.php?u=https%3A%2F%2Fdocs.cashfree.com%2Freference%2Forderpay&h=AT2PbFbrfwnt3znOaYe8wjq2g-CvXIog9xs1fqLByx0UojWZOVJVngUsZQAGWCrR6yzCyNPtb4Jt7rkmKrbhAPmbJuiM08-KFvA-8emqu2TYCT6zDVbKbJGUSepDkS76uz77Qd__JjYujQ)
...
Use the value in “tr” as the reference_id while setting up the [Parameters object](https://developers.facebook.com/docs/whatsapp/on-premises/payments-api/upi#paramobject) to send the payment message using WhatsApp API.
The value of “pa” is the merchant vpa that will be used for this transaction. The payment configuration name corresponding to the vpa returned should be used as payment_configuration while setting up the [Parameters object](https://developers.facebook.com/docs/whatsapp/on-premises/payments-api/upi#paramobject) to send the payment message using WhatsApp API.
In case the merchant vpa obtained from above does not match with any of the vpas set in the WhatsApp payment configuration, payment should be discontinued. Please reach out to Cashfree to confirm the updated vpa and update the [WhatsApp payment configuration](https://developers.facebook.com/docs/whatsapp/on-premises/payments-api/upi#before-you-start) accordingly.
...
Based on the payment_status in the webhook, [update the order status for the user using the WhatsApp API](https://developers.facebook.com/docs/whatsapp/on-premises/payments-api/upi#step-8--update-order-status).

[Reference Guide](https://l.facebook.com/l.php?u=https%3A%2F%2Fdocs.cashfree.com%2Fdocs%2Fpayment-webhooks&h=AT2PbFbrfwnt3znOaYe8wjq2g-CvXIog9xs1fqLByx0UojWZOVJVngUsZQAGWCrR6yzCyNPtb4Jt7rkmKrbhAPmbJuiM08-KFvA-8emqu2TYCT6zDVbKbJGUSepDkS76uz77Qd__JjYujQ)
...
Status api can be used as an alternative in case the webhook isn’t received within a certain timeframe. Based on the payment_status in the response, [update the order status for the user using the WhatsApp API](https://developers.facebook.com/docs/whatsapp/on-premises/payments-api/upi#step-8--update-order-status).

[Reference Doc](https://l.facebook.com/l.php?u=https%3A%2F%2Fdocs.cashfree.com%2Freference%2Fgetpaymentbyid&h=AT2PbFbrfwnt3znOaYe8wjq2g-CvXIog9xs1fqLByx0UojWZOVJVngUsZQAGWCrR6yzCyNPtb4Jt7rkmKrbhAPmbJuiM08-KFvA-8emqu2TYCT6zDVbKbJGUSepDkS76uz77Qd__JjYujQ)
...
[Reference Doc](https://l.facebook.com/l.php?u=https%3A%2F%2Fdocs.cashfree.com%2Freference%2Fcreaterefund&h=AT2PbFbrfwnt3znOaYe8wjq2g-CvXIog9xs1fqLByx0UojWZOVJVngUsZQAGWCrR6yzCyNPtb4Jt7rkmKrbhAPmbJuiM08-KFvA-8emqu2TYCT6zDVbKbJGUSepDkS76uz77Qd__JjYujQ)
...
Post order expiry, if no webhook was received, do a status check to ensure that the order expired and then [cancel the order at WhatsApp to update the user](https://developers.facebook.com/docs/whatsapp/on-premises/payments-api/upi#step-8--update-order-status).
...
So when a failed payment response is received from Cashfree, [update the status of order at WhatsApp to cancelled](https://developers.facebook.com/docs/whatsapp/on-premises/payments-api/upi#step-8--update-order-status). Post that a new payment message can be sent to the user to retry the payment.
